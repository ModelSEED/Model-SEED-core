...
sub simulatekomedialist {
	...
	if (defined($args->{compareTypes})) {
				...
				if (defined($args->{compareReferences}->[$i])) {
					if (defined($args->{compareTargets}->[$i])) {
						if ($args->{compareTypes}->[$i] eq "model") {
							my $refObj = $self->figmodel()->get_model($args->{compareReferences}->[$i]);
							...
							my $refObj = $self->figmodel()->get_media($args->{compareReferences}->[$i]);
							my $cmpObj = $self->figmodel()->get_media($args->{compareTargets}->[$i]);
							...
							my $refObj = $self->figmodel()->get_reaction($args->{compareReferences}->[$i]);
							my $cmpObj = $self->figmodel()->get_reaction($args->{compareTargets}->[$i]);
		...
		my $comparisonOutput = ["Type\tID\tChange\tNew FP\tNew FN\tNew CP\tNew CN\t0 to 1\t1 to 0\tNew FP\tNew FN\tNew CP\tNew CN\t0 to 1\t1 to 0"];
		my $changeTypes = ["new FP","new FN","new CP","new CN","0 to 1","1 to 0"];
		for (my $i=0; $i < @{$comparisonResults}; $i++) {
			my $line = $comparisonResults->[$i]->{type}."\t".$comparisonResults->[$i]->{id}."\t".$comparisonResults->[$i]->{change};
