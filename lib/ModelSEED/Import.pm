...
has om => (
    is      => 'rw',
    isa     => 'ModelSEED::ObjectManager',
    builder => '_makeObjectManager',
    lazy    => 1
);
has fm => (
    is      => 'rw',
    isa     => 'ModelSEED::FIGMODEL',
...
has typesToHashFns => (
    is      => 'rw',
    isa     => 'HashRef',
    builder => '_makeTypesToHashFns',
    lazy    => 1
);
has conversionFns => (
    is      => 'rw',
    isa     => 'HashRef',
    builder => '_makeConversionFns',
    lazy    => 1
);
has objectToQueryFns => (
    is      => 'rw',
    isa     => 'HashRef',
    builder => '_makeObjectToQueryFns',
    lazy    => 1
);
has _file_cache => ( is => 'rw', isa => 'HashRef', default => sub { return {} } );
has _cache => (
    is      => 'rw',
    isa     => 'ModelSEED::Import::Cache::Tiered',
    builder => '_makeCache',
    lazy    => 1
);
...
# elapsed() returns the time since the last call to
# the function. This is stored in the global $time variable.
my $time = time;
sub elapsed {
...
sub BUILD {
    ...
    if ( $self->clearOnStart ) {
        ...
        if ( $self->driver eq 'SQLite' ) {
            ...
            system( "cat lib/ModelSEED/ModelDB.sqlite | sqlite3 "
                  . $self->database );
        }
    }
    if ( defined( $self->expectedMisses )
        && -f abs_path( $self->expectedMisses ) )
    {
        $self->checker(
            ModelSEED::Import::Checker->new(
                { efFile => $self->expectedMisses }
            )
        );
    } else {
    ...
    $self->fm->authenticate(
        { username => "sdevoid", password => "marcopolo" } );
}
...
sub convert {
    ...
    confess "type or object not defined"
        unless (defined($type) && defined($obj));
    confess "no subroutine for $type"
        unless (defined($self->conversionFns->{$type}));
...
sub _makeTypesToHashFns {
    ...
    $f->{reactionset} = sub {
        return md5_hex($_[0]->id
                ...
                . ($_[0]->class || '')
                . join(',',
                sort map { $f->{reaction}->($_) } $_[0]->reactions));
    };
    $f->{compoundset} = sub {
        return md5_hex($_[0]->id
                . ($_[0]->name  || '')
                . ($_[0]->class || '')
                . join(',',
                sort map { $f->{compound}->($_) } $_[0]->compounds));
    };
    ...
    $f->{reaction_rule_transport} = sub {
        return md5_hex($f->{reaction}->($_[0]->reaction)
                . $_[0]->compartmentIndex
                . $f->{compound}->($_[0]->compound));
    };
    $f->{reaction_rule} = sub {
        return md5_hex(
                  $f->{reaction}->($_[0]->reaction)
                . $f->{compartment}->($_[0]->compartment)
                . join(',',
                sort map { $f->{reaction_rule_transport}->($_) }
                    $_[0]->reaction_rule_transports)
        );

    };
    $f->{reagent} = sub {
        my ($obj) = @_;
        return md5_hex($f->{reaction}->($obj->reaction)
                . $f->{compound}->($obj->compound)
                . $obj->compartmentIndex);
    };
    ...
    $f->{complex_role} = sub {
        return md5_hex(
            $f->{complex}->($_[0]->complex) . $f->{role}->($_[0]->role));
    };
    $f->{media} = sub {
    ...
    $f->{media_compound} = sub {
        return md5_hex(
            $f->{media}->($_[0]->media) . $f->{compound}->($_[0]->compound));
    };
    $f->{biochemistry} = sub {
        return md5_hex(
            join(',', sort map { $f->{media}->($_) } $_[0]->media)
                . join(',',
                sort map { $f->{compartment}->($_) } $_[0]->compartments)
                . join(',',
                sort map { $f->{compound}->($_) } $_[0]->compounds)
                . join(',',
                sort map { $f->{reaction}->($_) } $_[0]->reactions)
                . join(',',
                sort map { $f->{reactionset}->($_) } $_[0]->reactionsets)
                . join(',',
                sort map { $f->{compoundset}->($_) } $_[0]->compoundsets)
        );
    };
    $f->{mapping} = sub {
        return md5_hex(
                ...
                . join(',',
                sort map { $f->{reaction_rule}->($_) } $_[0]->reaction_rules)
                . join(',',
                sort map { $f->{reaction_rule}->($_) } $_[0]->reaction_rules)
    ...
    $f->{feature} = sub {
        return md5_hex($f->{genome}->($_[0]->genome)
                . $_[0]->id
                . $_[0]->start
                . $_[0]->stop);
    };
    $f->{annotation_feature} = sub {
        return md5_hex(
            $f->{feature}->($_[0]->feature) . $f->{role}->($_[0]->role));
    };
    $f->{annotation} = sub {
        return md5_hex(
                  $f->{genome}->($_[0]->genome) 
                . ($_[0]->name || '')
                . join(',',
                sort map { $f->{annotation_feature}->($_) }
                    $_[0]->annotation_features)
        );
    };
    $f->{model} = sub {
        return md5_hex(
    ...
    $f->{biomass_compound} = sub {
        return md5_hex($f->{compound}->($_[0]->compound)
                . $_[0]->coefficient
                . $f->{model_compartment}->($_[0]->model_compartment));
    };
    $f->{biomass} = sub {
        return md5_hex(
            $_[0]->id
                . join(',',
                sort map { $f->{biomass_compound}->($_) }
                    $_[0]->biomass_compounds)
        );
    };
    return $f;
};
...
sub _makeObjectToQueryFns {
    ...
    $f->{compound_alias}
        = sub { return {alias => $_[0]->alias, type => $_[0]->type} };
...
sub importBiochemistryFromDir {
    ...
    $self->checker->check("Imported biochemistry $username/$name [complete] ",
        elapsed(), $missed);
...
sub importMappingFromDir {
    ...
    for (my $i = 0; $i < $tables->{reactionRule}->size(); $i++) {
        ...
        unless (defined($cpx) && defined($hash)) {
            push(
                @{$missed->{reactionRule}},
                $row->{COMPLEX}->[0] . ":" . $row->{REACTION}->[0]
            );
            next;
        }
        my $RDB_reactionRule
            = $self->getOrCreateObject("reaction_rule", $hash);
...
sub importAnnotationFromDir {
    ...
    for (my $i = 0; $i < $tbl->size(); $i++) {
        ...
        $RDB_genome = $self->getGenomeObject($row->{GENOME}->[0])
        ...
        unless (defined($RDB_annotation)) {
            $RDB_annotation = $self->om->create_object("annotation",
                {genome => $RDB_genome});
        }
...
sub importModelFromDir {
    ...
    for (my $i = 0; $i < $size; $i++) {
        ...
        foreach my $dtr (@$dtrs) {
            ...
            my $mtr = $self->getOrCreateObject("model_transported_reagent",
                $hash);
        }
    }
    my $rdb_biomasses = [];
...
sub getDefaultCompartments {
    ...
    if (@$compartments == 0) {
        ...
        foreach my $old (@$oldCompartments) {
            ...
            my $obj = $self->getOrCreateObject("compartment", $hash)
                if (defined($hash->{id}));
            unless (defined($obj)) {
        ...
        foreach my $cmp (@$values) {
            if (!defined($defaultHierarchy->{$cmp->id})) {
                warn "No default compartment hierarchy for "
                    . $cmp->name . " ("
                    . $cmp->id . ")\n";
            }
        }
...
sub getDefaultMedia {
    ...
    if (@$media == 0) {
        ...
        foreach my $old (@$oldMedia) {
            ...
            my $mediaCpds = $self->fm->database->get_objects('mediacpd',
                {MEDIA => $old->id()});
            my $newMediaCpds = [];
...
sub getBiomass {
    ...
    my $oldBiomassCompounds
        = $self->fm->database->get_objects("cpdbof", {BIOMASS => $biomassId});
    ...
    my $biomassHash = {
        ...
        ? DateTime->from_epoch(epoch => $oldBiomass->modificationDate)
        : DateTime->now(),
        id   => $oldBiomass->id,
...
# 2A[e] + B => A + A[p] + C
    ...
    foreach my $part (@$parts) {
        ...
        if ($cmp ne $reactionCompartment) {
            unless (defined($seenCompartments->{$cmp})) {
                $seenCompartments->{$cmp}
                    = (scalar(keys %$seenCompartments) + 1);
            }
            my $index = $seenCompartments->{$cmp};
            $transports->{$cpd}->{$cmp} = 0
                unless (defined($transports->{$cpd}->{$cmp}));
    ...
    foreach my $part (@$parts) {
        ...
        if ($cmp ne $reactionCompartment) {
            ...
            my $importCoefficient
                = ($transports->{$cpd}->{$cmp} < 0) ? 1 : -1;
            my $isImport = ($transports->{$cpd}->{$cmp} < 0) ? 1 : 0;
            my $reagent = $self->convert(
                "reagent",
