...
sub finish {
	...
	if (defined($self->{_finishedfile} ne "NONE")) {
	    if ($self->{_finishedfile} =~ m/^\//) {
	        ...
	        ModelSEED::FIGMODEL::PrintArrayToFile($driv->{_figmodel}->{"database message file directory"}->[0].$self->{_finishedfile},[$message]);
	    }
	}
	exit();
}
...
sub completegapfillmodel {
    ...
    my $args = $self->check([
		["model",1],
		["remove gapfilled reactions",0,1],
		["run gapfilling",0,1],
		["start fresh",0,1],
		["queue",0,0]
	],[@Data]);
