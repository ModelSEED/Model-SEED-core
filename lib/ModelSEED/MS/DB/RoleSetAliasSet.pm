...
# SUBOBJECTS:
has roleSetAliases => (is => 'rw', isa => 'ArrayRef[HashRef]', default => sub { return []; }, type => 'child(RoleSetAlias)', metaclass => 'Typed', reader => '_roleSetAliases');
...
my $subobject_map = {roleSetAliases => 0};
sub _subobjects {
    my ($self, $key) = @_;
    if (defined($key)) {
...
# SUBOBJECT READERS:
around 'roleSetAliases' => sub {
    my ($orig, $self) = @_;
    return $self->_build_all_objects('roleSetAliases');
};
