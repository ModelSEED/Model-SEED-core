...
has rightHandSide => (is => 'rw', isa => 'Num', required => 1, type => 'attribute', metaclass => 'Typed');
has equalityType => (is => 'rw', isa => 'Str', required => 1, type => 'attribute', metaclass => 'Typed');
has index => (is => 'rw', isa => 'Int', required => 1, type => 'attribute', metaclass => 'Typed');
has primal => (is => 'rw', isa => 'Bool', required => 1, type => 'attribute', metaclass => 'Typed');
...
has dualVariable => (is => 'rw', isa => 'ModelSEED::MS::Constraint', type => 'link(FBAProblem,constraints,dualVariable_uuid)', metaclass => 'Typed', lazy => 1, builder => '_builddualVariable', weak_ref => 1);
...
sub _builddualVariable {
    my ($self) = @_;
    return $self->getLinkedObject('FBAProblem','constraints',$self->dualVariable_uuid());
}
