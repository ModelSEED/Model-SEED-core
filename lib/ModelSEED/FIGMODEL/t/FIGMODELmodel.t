...
$helper->copyProdModelState("Seed83333.1.16242");
...
# test general access routines
{
	...
	ok -e $public_model->directory().$public_model->id().".xml", "SBML file failed to print for model Seed83333.1";
}
# test general access routines on private-model
{
    my $private_model = $fm->get_model('Seed83333.1.16242');
    ok !defined($private_model), "model Seed83333.1.16242 is private, should have to login as reviewer to see!";
}
...
# PRIVATE MODEL TESTS
{
    # testing getting private model (user: reviewer, model: Seed83333.1.16242)
    $fm->authenticate({ username => "reviewer", password => "reviewer"});
    ok $fm->user() eq "reviewer", "a bit of meta-testing: we did log in correct?";
    my $private_model = $fm->get_model('Seed83333.1.16242');
    ok defined($private_model), "should actually get model Seed83333.1.16242";
    ok $private_model->version() == $private_model->ppo_version(), "we should get the cannonical version of Seed83333.1.16242, got " . $private_model->version();
}
...
# VERSIONING TESTS:
{
    ...
    $fm->authenticate({ username => "reviewer", password => "reviewer"});
    my $mdl = $fm->get_model("Seed83333.1.16242");
    my $mdl_db = $mdl->figmodel()->database();
     
    ok defined($mdl), "should actually get model Seed83333.1.16242";
    ok $mdl->isEditable(), "should be able to edit Seed83333.1.16242";
...
$helper->copyProdModelState("Seed83333.1.16242"); # restore from all changes we just made
...
# TEST plumbing commands: flatten(), increment(), restore()
{
    ...
    ok $private_model->version() == $private_model->ppo_version(),
        "we should get the cannonical version of Seed83333.1.16242, got " . $private_model->version();
    ok $private_model->isEditable(), "should be able to edit Seed83333.1.16242";
    ...
    $private_model->restore("Seed83333.1.16242.v0", 1);
...
#### Testing copyModel
{
    ...
    $helper->copyProdModelState("Seed83333.1.16242");
    $fm->authenticate({ username => "reviewer", password => "reviewer"});
    my $model_orig = $fm->get_model('Seed83333.1.16242');
    ok defined($model_orig), "should be able to get model Seed83333.1.16242 as reviewer";
    my $model_copy = $model_orig->copyModel({ owner => "devoid" });
    ok defined($model_copy), "should get model copy as reviewer, model successfully copied";
