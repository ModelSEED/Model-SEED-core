...
--CREATE DATABASE IF NOT EXISTS "ModelDB"; --DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci;
--USE `ModelDB` ;
...
CREATE  TABLE IF NOT EXISTS `genome` (
  ...
  `genes` INT NULL ,
  `gc` DOUBLE NULL ,
  `cellwall` CHAR(1) NULL ,
  `aerobicity` CHAR(1) NULL ,
  `media` CHAR(36) NOT NULL ,
  PRIMARY KEY (`uuid`) ,
  INDEX `genome_media_fk` (`media`) ,
  CONSTRAINT `genome_media_fk`
    FOREIGN KEY (`media` )
...
CREATE  TABLE IF NOT EXISTS `reaction_compound` (
  ...
  INDEX `reaction_compound_compartment_fk` (`compartment`) ,
  ...
  CONSTRAINT `reaction_compound_compound_fk`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `reaction_compound_compartment_fk`
